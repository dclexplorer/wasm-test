...
impl Runtime {
    ...
    pub fn new() -> Result<Runtime, JsValue> {
    ...
    pub fn prepare_sdk7_sandbox_context(&self, can_use_websocket: bool, can_use_fetch: bool, preview_mode: bool) -> Result<Object, JsValue> {
        let context = Object::new();
        
        // Log function placeholder - wrap in parentheses to make it an expression
        let log_func_code = r#"
            (function(message) {
                console.log('[Sandbox]', message);
            })
        "#;
        let log_func = eval(log_func_code)?;
        Reflect::set(&context, &JsValue::from_str("log"), &log_func)?;
        
        // For now, we'll just note the permissions in the context
        // WebSocket and fetch restrictions will be implemented later
        Reflect::set(&context, &JsValue::from_str("canUseWebsocket"), &JsValue::from_bool(can_use_websocket))?;
        Reflect::set(&context, &JsValue::from_str("canUseFetch"), &JsValue::from_bool(can_use_fetch))?;
        Reflect::set(&context, &JsValue::from_str("previewMode"), &JsValue::from_bool(preview_mode))?;
